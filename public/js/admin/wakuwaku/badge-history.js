document.addEventListener('DOMContentLoaded', () => {
    const page = document.querySelector('.badge-history-page');
    if (!page) return;
    ...
    const badgeLookupUrl = page.dataset.badgeLookupUrl;
    ...
    const filters = {
        ...
        badge_id: document.getElementById('historyBadge'), category_id: document.getElementById('historyCategory'),
    ...
    const renderRows = rows => {
        ...
        body.innerHTML = rows.map(row => `
            <tr class="history-data-row" data-row-history-id="${row.id}">
                ...
                <td><div class="history-badge-cell">${row.badge_image_path ? `<img class="history-badge-image" src="${esc(row.badge_image_path)}" alt="" onerror="this.replaceWith(Object.assign(document.createElement('span'),{className:'history-badge-image',innerHTML:'<i class=\'fas fa-medal\'></i>'}))">` : '<span class="history-badge-image"><i class="fas fa-medal"></i></span>'}<div class="history-badge"><strong>${esc(row.badge_name)}</strong><small>${esc(row.badge_code || '')}</small></div></div></td>
                ...
                <td class="history-operation-column"><div class="history-row-actions"><button class="history-detail-button" data-history-id="${row.id}">詳細</button>${row.status === 'active' ? `<button class="history-remove-button" data-remove-id="${row.id}" data-student="${esc(row.student_name)}" data-badge="${esc(row.badge_name)}">取り外す</button>` : `<button class="history-regrant-button" data-regrant-id="${row.id}" data-student="${esc(row.student_name)}" data-badge="${esc(row.badge_name)}">再付与</button>`}</div></td>
            </tr>`).join('');
    };
    ...
    const operationModal = document.getElementById('badgeOperationModal');
    ...
    const badgeLookup = createLookup({
        inputId: 'manualBadgeLookup', hiddenId: 'manualBadgeId', selectedId: 'manualBadgeSelected', resultsId: 'manualBadgeResults',
        url: badgeLookupUrl, emptyMessage: '手動付与できるバッジが見つかりません。'
    });
    ...
    body.addEventListener('click', async event => {
        ...
        if (remove) return openOperation('remove', remove.dataset.removeId, remove.dataset.student, remove.dataset.badge);
        const regrant = event.target.closest('[data-regrant-id]');
        if (regrant) return openOperation('regrant', regrant.dataset.regrantId, regrant.dataset.student, regrant.dataset.badge);
    });

    const openDetail = data => {
        const grant = data.grant;
        document.getElementById('historyDetailSubtitle').textContent = `${grant.student_name} / ${grant.badge_name}`;
        document.getElementById('historyDetailHero').innerHTML = `${grant.badge_image_path ? `<img src="${esc(grant.badge_image_path)}" alt="">` : '<span><i class="fas fa-medal"></i></span>'}<div><strong>${esc(grant.badge_name)}</strong><small>${esc(grant.badge_code || '')}</small><p>${esc(grant.student_name)}（${esc(grant.student_code || '-') }）</p></div>`;
    ...
    const openOperation = (type, id, student, badge) => {
        const form = document.getElementById('badgeOperationForm'); form.reset();
        form.elements.grant_id.value = id; form.elements.operation_type.value = type;
        document.getElementById('badgeOperationTitle').textContent = type === 'remove' ? 'バッジを取り外す' : 'バッジを再付与する';
        document.getElementById('badgeOperationSubtitle').textContent = `${student} / ${badge}`;
        ...
        document.getElementById('badgeOperationSubmit').textContent = type === 'remove' ? '取り外す' : '再付与する';
        document.getElementById('badgeOperationWarning').classList.toggle('is-danger', type === 'remove');
        document.getElementById('badgeOperationError').textContent=''; setModal(operationModal, true);
    };
    ...
    document.getElementById('manualGrantForm').addEventListener('submit', async event => {
        ...
        if (!badgeLookup.hasValue()) { error.textContent = 'バッジを検索結果から選択してください。'; document.getElementById('manualBadgeLookup').focus(); return; }
    ...
    document.getElementById('badgeOperationForm').addEventListener('submit', async event => {
        ...
        if (!form.elements.confirmed.checked) { document.getElementById('badgeOperationError').textContent='対象と操作内容を確認してください。'; submit.disabled=false; return; }
        if (type === 'remove' && form.elements.reason_code.value === 'other' && !form.elements.reason_detail.value.trim()) { document.getElementById('badgeOperationError').textContent='「その他」を選択した場合は理由の詳細を入力してください。'; submit.disabled=false; return; }
        ...
        catch(e){ document.getElementById('badgeOperationError').textContent=e.message; } finally { submit.disabled=false; }
    });
    load();
});
