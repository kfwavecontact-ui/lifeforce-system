...
@section('title', '称号獲得履歴')

@section('content')
<div class="title-history-page"
     data-list-url="{{ route('admin.wakuwaku.titles.history.list') }}"
     data-detail-url-base="{{ url('/admin/wakuwaku/titles/history') }}"
     data-grant-url="{{ route('admin.wakuwaku.titles.history.grant') }}"
     data-student-lookup-url="{{ route('admin.wakuwaku.titles.history.lookup.students') }}"
     data-title-lookup-url="{{ route('admin.wakuwaku.titles.history.lookup.titles') }}">
    <div class="master-page-header title-history-header">
        <div>
            <h1>獲得履歴</h1>
            <p>生徒への称号付与・取り外し・再付与の履歴を確認します。</p>
        </div>
    ...
    <main class="title-history-main">
        <section class="title-history-summary-grid" aria-label="獲得履歴集計">
        ...
        <section class="title-history-filter-card">
            <div class="history-filter-grid">
                <label class="history-keyword"><span>検索</span><div><i class="fas fa-search"></i><input id="historyKeyword" type="text" placeholder="生徒名・生徒コード・称号名・理由"></div></label>
                ...
                <label><span>称号</span><select id="historyTitle"><option value="all">すべて</option>@foreach($titles as $title)<option value="{{ $title->id }}">{{ $title->name }}</option>@endforeach</select></label>
                ...
                <label><span>並び順</span><select id="historySort"><option value="acquired_desc">獲得日時が新しい順</option><option value="acquired_asc">獲得日時が古い順</option><option value="student_asc">生徒名 昇順</option><option value="student_desc">生徒名 降順</option><option value="title_asc">称号名 昇順</option><option value="title_desc">称号名 降順</option><option value="status_asc">状態 昇順</option><option value="status_desc">状態 降順</option></select></label>
            </div>
            <div class="history-filter-actions"><button type="button" id="historyResetButton"><i class="fas fa-rotate-left"></i> 条件クリア</button></div>
        </section>

        <section class="title-history-table-card">
            ...
            <div class="history-table-scroll">
                <table class="history-table">
                    <thead><tr>
                        ...
                        <th>教室</th><th><button data-sort="title">称号 <span>↕</span></button></th><th><button data-sort="category">カテゴリ <span>↕</span></button></th><th><button data-sort="series">シリーズ <span>↕</span></button></th><th><button data-sort="method">付与方法 <span>↕</span></button></th><th><button data-sort="operator">付与者 <span>↕</span></button></th>
...
<div class="history-modal" id="manualGrantModal" aria-hidden="true">
    ...
    <section class="history-modal-dialog history-operation-dialog" role="dialog" aria-modal="true" aria-labelledby="manualGrantTitle">
        <header><div><h2 id="manualGrantTitle">称号を手動付与</h2><p>対象生徒と称号を選択してください。</p></div><button type="button" data-close-manual-modal aria-label="閉じる">×</button></header>
        <form id="manualGrantForm">
            <div class="history-modal-body history-form-grid">
                ...
                <label class="history-lookup-field">
                    <span>称号 <b>必須</b></span>
                    <input type="hidden" name="title_id" id="manualTitleId">
                    <div class="history-lookup" data-lookup="title">
                        <div class="history-lookup-input-wrap"><i class="fas fa-search"></i><input type="search" id="manualTitleLookup" autocomplete="off" placeholder="称号コード・名称で検索"></div>
                        <div class="history-lookup-selected" id="manualTitleSelected" hidden></div>
                        <div class="history-lookup-results" id="manualTitleResults" hidden></div>
                    </div>
                </label>
...
<div class="history-modal" id="titleOperationModal" aria-hidden="true">
    <div class="history-modal-backdrop" data-close-operation-modal></div>
    <section class="history-modal-dialog history-operation-dialog" role="dialog" aria-modal="true" aria-labelledby="titleOperationTitle">
        <header><div><h2 id="titleOperationTitle">称号操作</h2><p id="titleOperationSubtitle"></p></div><button type="button" data-close-operation-modal aria-label="閉じる">×</button></header>
        <form id="titleOperationForm">
            <input type="hidden" name="grant_id"><input type="hidden" name="operation_type">
            <div class="history-modal-body history-form-grid">
                <div class="history-operation-warning history-form-wide" id="titleOperationWarning"><i class="fas fa-triangle-exclamation"></i><div><strong>この操作は履歴に記録されます。</strong><p>対象生徒と称号を確認し、理由を入力してから実行してください。</p></div></div>
                ...
                <label class="history-check history-form-wide history-confirm-check"><input type="checkbox" name="confirmed" value="1" required><span>対象と操作内容を確認しました</span></label><div class="history-form-error history-form-wide" id="titleOperationError"></div>
            </div>
            <footer><button type="button" data-close-operation-modal>キャンセル</button><button type="submit" class="history-submit-button" id="titleOperationSubmit">実行する</button></footer>
        </form>
    </section>
</div>
        ...
        <div class="history-modal-body">
            ...
            <div class="history-csv-grid" id="historyCsvColumns">
                @foreach([
                    ...
                    'title_code'=>'称号コード','title_name'=>'称号名','category_name'=>'カテゴリ','series_name'=>'シリーズ',
...
<link rel="stylesheet" href="{{ asset('css/admin/wakuwaku/title-history.css') }}">
<script src="{{ asset('js/admin/wakuwaku/title-history.js') }}"></script>
@endsection
