<form class="routine-search" method="GET" action="{{ route('admin.system.routines') }}">
    ...
    <div class="routine-search-grid routine-grid">
        ...
        <label><span>難易度</span><select name="routine_difficulty"><option value="all">すべて</option>@foreach(['非常に低い','低い','標準','高い','非常に高い'] as $option)<option value="{{ $option }}" @selected(request('routine_difficulty') === $option)>{{ $option }}</option>@endforeach</select></label>
    ...
    <div class="routine-search-actions"><button type="submit">検索</button><a href="{{ route('admin.system.routines', ['tab' => 'routines']) }}">リセット</a></div>
</form>
