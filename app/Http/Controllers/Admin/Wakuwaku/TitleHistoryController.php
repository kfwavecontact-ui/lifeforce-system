...
use App\Services\Reward\TitleGrantService;
use DomainException;
...
/**
 * わくわく ＞ 称号 ＞ 獲得履歴画面を管理するController。
 *
 * 役割:
 * - 生徒への称号付与単位の履歴一覧、集計、検索、CSV出力
 * - 付与・取り外し・再付与イベントの詳細表示
 *
 * 利用DB:
 * - student_titles（参照）
 * - title_histories（参照）
 * - students / schools / titles / title_categories / title_series / users（参照）
 */
class TitleHistoryController extends Controller
{
    ...
    {
        return view('admin.wakuwaku.titles.history', [
            'titles' => Title::orderBy('display_order')->orderBy('id')->get(['id', 'name', 'code']),
            'categories' => TitleCategory::where('is_active', true)->orderBy('display_order')->get(['id', 'name']),
            'series' => TitleSeries::where('is_active', true)->orderBy('display_order')->orderBy('id')->get(['id', 'name']),
    ...
    /**
     * 手動付与モーダル用の称号Lookup。
     * コード・名称・説明で検索し、有効な称号を最大20件だけ返す。
     * 自動付与専用称号も、管理者による手動付与候補として表示する。
     */
    public function titleLookup(Request $request): JsonResponse
    {
        $keyword = trim((string) $request->query('q', ''));
         ...
         * PHP側で同じ規則に正規化して照合する。
         * 称号件数が増えても全件取得にならないよう、有効称号を表示順で最大500件に限定する。
         */
        ...
        $titles = Title::query()
            ...
            ->limit(500)
            ->get()
            ->filter(function (Title $title) use ($normalizedKeyword, $normalize): bool {
                if ($normalizedKeyword === '') {
                    return false;
                }

                foreach ([$title->code, $title->name, $title->description] as $candidate) {
        ...
        return response()->json([
            'items' => $titles->map(fn (Title $title) => [
                'id' => $title->id,
                'primary' => $title->name,
                'secondary' => trim(
                    ($title->code ?? '')
                    . ' / ' . ($title->category?->name ?? 'カテゴリ未設定')
                    . ' / ' . ($title->series?->name ?? 'シリーズ未設定'),
                    ' /'
                ),
            ])->values(),
        ]);
    ...
    public function list(Request $request): JsonResponse|StreamedResponse
    {
        ...
        return response()->json([
            'rows' => $rows->map(fn (StudentTitle $grant) => $this->serializeGrant($grant))->values(),
            'summary' => [
                'total_grants' => StudentTitle::count(),
                'active_holdings' => StudentTitle::where('status', 'active')->count(),
                'monthly_grants' => StudentTitle::whereBetween('acquired_at', [$monthStart, $monthEnd])->count(),
                'monthly_removals' => StudentTitle::whereBetween('removed_at', [$monthStart, $monthEnd])->count(),
                'monthly_regrants' => StudentTitle::where('grant_method', 'regrant')
    ...
    public function grant(Request $request): JsonResponse
    {
        ...
        try {
            $grant = $this->titleGrantService->grant((int) $validated['student_id'], (int) $validated['title_id'], [
                'grant_method' => 'manual',
                'operator_id' => auth()->id(),
        ...
        return response()->json(['message' => '称号を手動付与しました。', 'grant_id' => $grant->id], 201);
    }

    public function remove(Request $request, StudentTitle $studentTitle): JsonResponse
    {
        $validated = $request->validate([
        ...
        try {
            $grant = $this->titleGrantService->remove($studentTitle->id, [
                'operator_id' => auth()->id(),
        ...
        return response()->json(['message' => '称号を取り外しました。', 'grant_id' => $grant->id]);
    }

    public function regrant(Request $request, StudentTitle $studentTitle): JsonResponse
    {
        $validated = $request->validate([
            'reason' => ['required', 'string', 'max:1000'],
            'acquired_at' => ['nullable', 'date'],
        ...
        try {
            $grant = $this->titleGrantService->regrant($studentTitle->id, [
                'operator_id' => auth()->id(),
                'reason' => $validated['reason'],
                'acquired_at' => $validated['acquired_at'] ?? null,
                'notify' => (bool) ($validated['notify'] ?? false),
        ...
        return response()->json(['message' => '称号を再付与しました。', 'grant_id' => $grant->id], 201);
    }

    public function show(StudentTitle $studentTitle): JsonResponse
    {
        $studentTitle->load([
            'student.school',
            'title.category',
        ...
        return response()->json([
            'grant' => $this->serializeGrant($studentTitle),
            'events' => $studentTitle->events->map(function ($event) {
                return [
                    'id' => $event->id,
    ...
    private function buildQuery(Request $request): Builder
    {
            ...
            $query->where(function (Builder $q) use ($keyword) {
                $q->whereHas('student', function (Builder $studentQuery) use ($keyword) {
                    $studentQuery->where('student_code', 'like', "%{$keyword}%")
                        ...
                        ->orWhere('first_name', 'like', "%{$keyword}%")
                        ->orWhereRaw("CONCAT(last_name, ' ', first_name) LIKE ?", ["%{$keyword}%"]);
                })->orWhereHas('title', function (Builder $titleQuery) use ($keyword) {
                    $titleQuery->where('code', 'like', "%{$keyword}%")
        ...
        $this->applyExactFilter($query, 'title_id', $request->query('title_id'));
        ...
        if (($categoryId = $request->query('category_id')) && $categoryId !== 'all') {
            $query->whereHas('title', fn (Builder $q) => $q->where('title_category_id', $categoryId));
        }
        if (($seriesId = $request->query('series_id')) && $seriesId !== 'all') {
            $query->whereHas('title', fn (Builder $q) => $q->where('title_series_id', $seriesId));
        }
        ...
        match ($sort) {
            ...
            'student_asc' => $query->orderByRaw("(SELECT CONCAT(s.last_name, s.first_name) FROM students s WHERE s.id = student_titles.student_id) ASC NULLS LAST"),
            'student_desc' => $query->orderByRaw("(SELECT CONCAT(s.last_name, s.first_name) FROM students s WHERE s.id = student_titles.student_id) DESC NULLS LAST"),
            'title_asc' => $query->orderByRaw('(SELECT b.name FROM titles b WHERE b.id = student_titles.title_id) ASC NULLS LAST'),
            'title_desc' => $query->orderByRaw('(SELECT b.name FROM titles b WHERE b.id = student_titles.title_id) DESC NULLS LAST'),
            'category_asc' => $query->orderByRaw('(SELECT bc.name FROM titles b LEFT JOIN title_categories bc ON bc.id = b.title_category_id WHERE b.id = student_titles.title_id) ASC NULLS LAST')->orderByDesc('acquired_at'),
            'category_desc' => $query->orderByRaw('(SELECT bc.name FROM titles b LEFT JOIN title_categories bc ON bc.id = b.title_category_id WHERE b.id = student_titles.title_id) DESC NULLS LAST')->orderByDesc('acquired_at'),
            'series_asc' => $query->orderByRaw('(SELECT bs.name FROM titles b LEFT JOIN title_series bs ON bs.id = b.title_series_id WHERE b.id = student_titles.title_id) ASC NULLS LAST')->orderByDesc('acquired_at'),
            'series_desc' => $query->orderByRaw('(SELECT bs.name FROM titles b LEFT JOIN title_series bs ON bs.id = b.title_series_id WHERE b.id = student_titles.title_id) DESC NULLS LAST')->orderByDesc('acquired_at'),
            ...
            'operator_asc' => $query->orderByRaw('(SELECT u.name FROM users u WHERE u.id = student_titles.granted_by) ASC NULLS FIRST')->orderByDesc('acquired_at'),
            'operator_desc' => $query->orderByRaw('(SELECT u.name FROM users u WHERE u.id = student_titles.granted_by) DESC NULLS LAST')->orderByDesc('acquired_at'),
    ...
    private function serializeGrant(StudentTitle $grant): array
    {
        ...
        return [
            ...
            'title_id' => $grant->title_id,
            'title_code' => $grant->title?->code,
            'title_name' => $grant->title?->name ?? '削除済み称号',
            'title_image_path' => $this->resolveImageUrl($grant->title?->image_path),
            'category_name' => $grant->title?->category?->name ?? '未設定',
            'series_name' => $grant->title?->series?->name ?? '未設定',
            'status' => $status,
    ...
    {
        $filename = 'title-acquisition-history-' . now()->format('Ymd-His') . '.csv';
        $available = [
            'id' => 'ID', 'acquired_at' => '獲得日時', 'student_code' => '生徒コード',
            'student_name' => '生徒名', 'school_name' => '教室', 'title_code' => '称号コード',
            'title_name' => '称号名', 'category_name' => 'カテゴリ', 'series_name' => 'シリーズ',
